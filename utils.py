...
def get_vocab(train_set, test_set, val_set):
    """
    ...
    wid = 2
    for record in train_set + test_set + val_set:
...
def build_dataset(ds_name, input_win=1, tagging_schema='BIO', stm_win=1):
    ...
    train_path = '%s/train.txt' % ds_name
    test_path = '%s/test.txt' % ds_name
    val_path = '%s/dev.txt' % ds_name
    ...
    vocab, char_vocab = get_vocab(train_set=train_set, test_set=test_set, val_set=val_set)
...
def load_embeddings(path, vocab, ds_name, emb_name):
    ...
    if os.path.exists(pkl):
        ...
        if not os.path.exists('./embeddings'):
            os.mkdir('./embeddings')
        emb_path = f'./embeddings/{emb_name}.pkl'
